...
const waitForExit = (child, milliseconds) => new Promise((resolveExit) => {
  if (!child || child.exitCode !== null || child.signalCode !== null) return resolveExit(true)
  const timeout = setTimeout(() => resolveExit(false), milliseconds)
  child.once("close", () => { clearTimeout(timeout); resolveExit(true) })
})
...
const run = async (command, args, { cwd = root, env, timeoutMs = 60_000 } = {}) => {
  ...
  if (result.code !== 0) throw new Error(`${command} ${args.join(" ")} exited with ${result.code ?? result.signal}${stderr ? `: ${scrub(stderr)}` : ""}`)
  return { stdout, stderr }
}

const scrub = (value) => value.replace(/ab_[A-Za-z0-9_-]+/g, "[REDACTED_TOKEN]")
...
const releaseLock = async () => {
  if (!ownsLock) return
  try {
    const record = JSON.parse(await readFile(join(lockPath, "owner.json"), "utf8"))
    if (record?.pid === process.pid && record?.nonce === lockNonce) await rm(lockPath, { recursive: true, force: true })
  } finally { ownsLock = false }
}

async function reservePort() {
  const listener = createTcpServer()
  await new Promise((resolveListen, rejectListen) => { listener.once("error", rejectListen); listener.listen({ host: "127.0.0.1", port: 0 }, resolveListen) })
...
const startServer = async ({ retryPort = true } = {}) => {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    const child = spawnOwned("mix", ["run", "--no-halt"], { cwd: serverDir, env: environment(), stdio: ["ignore", "pipe", "pipe"] })
...
const startControl = async () => {
  ...
  control = createServer(async (request, response) => {
    if (request.method !== "POST" || request.url !== "/restart" || request.headers["x-autoboard-e2e-control"] !== controlToken) {
      response.writeHead(404).end(); return
    }
    try { await restartServer(); response.writeHead(204).end() } catch (error) { response.writeHead(500).end(scrub(String(error))) }
  })
  await new Promise((resolveListen, rejectListen) => { control.once("error", rejectListen); control.listen({ host: "127.0.0.1", port: 0 }, resolveListen) })
  const address = control.address()
  if (!address || typeof address === "string") throw new Error("could not bind the e2e control server")
...
try {
  ...
  if (process.argv.includes("--lock-probe")) {
    ...
    const slow = spawnOwned(process.execPath, ["-e", "setInterval(() => {}, 1000)"], { stdio: "ignore" })
    process.stdout.write(JSON.stringify({ tempDir, childPid: slow.pid }) + "\n")
    await new Promise(() => {})
  } else {
    requireBuildArtifacts()
    port = await reservePort()
    await run("mix", ["ecto.reset"], { cwd: serverDir, env: environment() })
    await run("mix", ["autoboard.setup"], { cwd: serverDir, env: environment() })
    const { stdout } = await run("mix", ["autoboard.token.create", "--actor", "codex"], { cwd: serverDir, env: environment() })
    ...
    await run("corepack", ["pnpm", "--filter", "@autoboard/e2e", "exec", "vitest", "run", "--config", "vitest.config.ts"], {
      env: { ...environment(), AUTOBOARD_E2E_SOCKET: join(tempDir, "autoboard.sock"), AUTOBOARD_E2E_TOKEN: token, AUTOBOARD_E2E_URL: `http://127.0.0.1:${port}`, AUTOBOARD_E2E_ATTACHMENT: attachmentPath, AUTOBOARD_E2E_CONTROL_URL: controlUrl, AUTOBOARD_E2E_CONTROL_TOKEN: controlToken },
      timeoutMs: 90_000,
    })
  }
} catch (error) {
