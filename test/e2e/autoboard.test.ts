...
const restartServer = async () => {
  const response = await fetch(`${controlUrl}/restart`, {
    method: "POST",
    headers: { "x-autoboard-e2e-control": controlToken },
  });
...
describe("Autoboard MCP to browser acceptance", () => {
  test("drives the complete read-only live-board loop", async () => {
    const project = await tool<Project>("create_project", {
      key: "AUTO",
      name: "Autoboard acceptance",
      description: "Created over MCP",
    });
    const parent = await tool<Ticket>("create_ticket", {
      project_id: project.id,
      title: "Parent work",
      status: "ready",
      assignee: "codex",
    });
    const subtask = await tool<Ticket>("create_ticket", {
      project_id: project.id,
    ...
    const blocker = await tool<Ticket>("create_ticket", {
      project_id: project.id,
      title: "Blocker",
      status: "backlog",
      assignee: "codex",
    });
    const blocked = await tool<Ticket>("create_ticket", {
      project_id: project.id,
      title: "Blocked work",
      status: "ready",
      assignee: "codex",
    ...
    await tool<Ticket>("add_dependency", {
      blocked_ticket_id: blocked.id,
      blocker_ticket_id: blocker.id,
      expected_revision: blocked.revision,
    });
    await tool("add_comment", {
      ticket_id: subtask.id,
      body: "A durable **MCP** comment.",
    });
    await tool("add_attachment_from_path", {
      ticket_id: subtask.id,
      path: attachmentPath,
    });

    const actionable = await tool<{ tickets: Ticket[] }>(
      "list_actionable_tickets",
      { project_id: project.id },
    );
    expect(actionable.tickets.map((ticket) => ticket.identifier)).toEqual([
      codex.identifier,
    ]);
    expect(actionable.tickets.map((ticket) => ticket.identifier)).not.toContain(
      human.identifier,
    );
    expect(actionable.tickets.map((ticket) => ticket.identifier)).not.toContain(
      parent.identifier,
    ...
    await tool<Ticket>("update_ticket", {
      ticket_id: codex.id,
    ...
    const freshBlocked = await tool<Ticket>("get_ticket", {
      ticket_id: blocked.id,
    });
    await failedTool(
      "add_dependency",
      {
        blocked_ticket_id: blocker.id,
        blocker_ticket_id: freshBlocked.id,
        expected_revision: blocker.revision,
      },
      "dependency_cycle",
    );

    const httpBoard = await fetch(`${baseUrl}/api/v1/projects/AUTO/board`).then(
      async (response) => {
    ...
    expect(
      httpBoard.columns.ready.map((ticket) => ticket.identifier),
    ).toContain(codex.identifier);
    const httpTicket = await fetch(
      `${baseUrl}/api/v1/tickets/${subtask.identifier}`,
    ).then(async (response) => {
      expect(response.status).toBe(200);
      return response.json() as Promise<{
        title: string;
        comments: Array<{ body: string }>;
        attachments: Array<{ original_filename: string }>;
      }>;
    });
    expect(httpTicket).toMatchObject({
      title: "Subtask",
      comments: [{ body: "A durable **MCP** comment." }],
      attachments: [{ original_filename: "note.txt" }],
    });
    ...
    await page
      .getByRole("link", { name: "Autoboard acceptance", exact: true })
      .waitFor({ state: "visible" });
    await page.goto(`${baseUrl}/projects/AUTO`, {
      waitUntil: "domcontentloaded",
    });
    await page
      .getByRole("heading", { name: "Autoboard acceptance" })
      .waitFor({ state: "visible" });
    await page
    ...
    expect(
      await page
        .locator(
          "form, input, textarea, select, button, [contenteditable=true], [draggable=true]",
        )
        .count(),
    ).toBe(0);
    ...
    page.on("request", (request) => {
      if (new URL(request.url()).pathname.startsWith("/api/v1"))
        replayApiMethods.push(request.method());
    });
    await page.goto(`${baseUrl}/projects/AUTO`, {
      waitUntil: "domcontentloaded",
    });
    ...
    await page.evaluate(() => {
      window.name = "autoboard-e2e-live-marker";
    });
    ...
    await restartMcp();
    await expect
      .poll(() => blockedSseReconnects, { timeout: 10_000 })
      .toBeGreaterThan(0);
    const persisted = await tool<{
      comments: Array<{ body: string }>;
      attachments: Array<{ original_filename: string }>;
      activity: unknown[];
    }>("get_ticket", { ticket_id: subtask.id });
    ...
    const freshCodex = await tool<Ticket>("get_ticket", {
      ticket_id: codex.id,
    });
    const inProgress = await tool<Ticket>("transition_ticket", {
      ticket_id: freshCodex.id,
      expected_revision: freshCodex.revision,
      status: "in_progress",
    });
    expect(
      await page
        .getByRole("region", { name: "Ready tickets" })
        .getByText("Codex leaf", { exact: true })
        .count(),
    ).toBe(1);
    ...
    await page
      .getByRole("region", { name: "In progress tickets" })
      .getByText("Codex leaf", { exact: true })
      .waitFor({ state: "visible" });
    expect(page.url()).toBe(beforeTransitionUrl);
    expect(await page.evaluate(() => window.name)).toBe(
      "autoboard-e2e-live-marker",
    );
    const done = await tool<Ticket>("transition_ticket", {
      ticket_id: inProgress.id,
      expected_revision: inProgress.revision,
      status: "done",
    });
    await page
      .getByRole("region", { name: "Done tickets" })
      .getByText("Codex leaf", { exact: true })
      .waitFor({ state: "visible" });
