...
const CurrentJson = Schema.Union(
  ProjectJsonSchema,
  TicketSummaryJsonSchema,
  TicketDetailJsonSchema,
);
...
const simple = Schema.Union(
  ...simpleKinds.map((kind) =>
    exactStruct({
...
const validation = exactStruct({
  kind: Schema.Literal("validation_failed"),
  message: Schema.String,
  fields: Fields,
});
const validationJson = Schema.Struct({
  kind: Schema.Literal("validation_failed"),
  message: Schema.String,
  fields: Fields,
});
const revision = exactStruct({
  kind: Schema.Literal("revision_conflict"),
  message: Schema.String,
  fields: Fields,
  current: Current,
});
const revisionJson = Schema.Struct({
  kind: Schema.Literal("revision_conflict"),
  message: Schema.String,
  fields: Fields,
  current: CurrentJson,
...
const methodNotFound = exactStruct({
  kind: Schema.Literal("method_not_found"),
});
const internal = exactStruct({
  kind: Schema.Literal("internal_error"),
  correlation_id: Schema.String,
});
const invalidRequestJson = Schema.Struct({
  kind: Schema.Literal("invalid_request"),
});
const methodNotFoundJson = Schema.Struct({
  kind: Schema.Literal("method_not_found"),
});
const internalJson = Schema.Struct({
  kind: Schema.Literal("internal_error"),
  correlation_id: Schema.String,
});

const envelope = <D extends Schema.Schema.Any>(code: number, data: D) =>
  exactStruct({
...
const envelopeJson = <D extends Schema.Schema.Any>(code: number, data: D) =>
  Schema.Struct({
    jsonrpc: Schema.Literal("2.0"),
    id: Schema.Union(Schema.String, Schema.Number, Schema.Null),
    error: Schema.Struct({
...
export const RpcEnvelopeFailure = Schema.Union(
  envelope(-32600, invalidRequest),
  envelope(-32601, methodNotFound),
  envelope(-32602, validation),
  envelope(-32010, RpcFailure),
  envelope(-32010, internal),
);
export const RpcEnvelopeFailureJsonSchema = Schema.Union(
  envelopeJson(-32600, invalidRequestJson),
  envelopeJson(-32601, methodNotFoundJson),
  envelopeJson(-32602, validationJson),
  envelopeJson(-32010, RpcFailureJsonSchema),
  envelopeJson(-32010, internalJson),
);
