...
import {
  RpcEnvelopeFailureJsonSchema,
  RpcFailureJsonSchema,
  RpcSuccessJsonSchema,
} from "./rpc.js";
...
const schemas = {
  ...
  "rpc-envelope-failure.schema.json": RpcEnvelopeFailureJsonSchema,
...
const xemaCompatible = (value: unknown): unknown => {
  ...
  if (object.$id === "/schemas/unknown" || object.$id === "/schemas/any")
    return true;
...
await Promise.all(
  Object.entries(schemas as Record<string, Schema.Schema.Any>).map(
    async ([filename, schema]) => {
      ...
      const document = xemaCompatible(
        OpenApiJsonSchema.make(schema, { target: "jsonSchema7" }),
      );
      await writeFile(
