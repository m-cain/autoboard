...
export const AppShell = ({ triageCount, projects, children }: Props) => (
  <div className="app-shell">
    <header className="app-header">
      <Link className="wordmark" to="/projects">
        Autoboard
      </Link>
      <Link className="triage-link" to="/triage">
        Triage ({triageCount})
      </Link>
    </header>
...
type RootData = {
  readonly projects: {
    readonly active: Project[];
    readonly archived: Project[];
  };
...
export const RouterAppShell = () => {
  ...
  const ticketData = matches.find((match) => match.id === "ticket")
    ?.loaderData as TicketRouteData | undefined;
  ...
  const background =
    ...
    "backgroundLocation" in location.state
      ...
      : undefined;
  const drawer = Boolean(
    ticket &&
    typeof background === "object" &&
    background !== null &&
    "pathname" in background &&
    background.pathname ===
      `/projects/${encodeURIComponent(ticket.project.key)}`,
  );
  current.current = {
    pathname: location.pathname,
    projects,
    ticket: ticket ? { ...ticket, relatedTicketIds } : undefined,
    drawer,
    rootTriageTicketIds,
  };
      ...
      relevant: (event) =>
        isActivityRelevant(
          ...
          current.current.ticket
            ? { ...current.current.ticket, drawer: current.current.drawer }
            : undefined,
          current.current.rootTriageTicketIds,
        ),
      revalidate,
  ...
  return (
    <AppShell
      triageCount={data.triage.tickets.length}
