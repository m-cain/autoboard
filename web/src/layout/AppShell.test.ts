...
describe("live revalidation", () => {
  it("revalidates root data only for mutations that can change projects or triage", () => {
    ...
    const crossProject = { ...event, project_id: otherProject, ticket_id: otherTicket }
    const rootTriageTicketIds = [ticketId]

    for (const eventType of ["project.created", "project.updated", "project.archived", "project.restored"]) {
      expect(isActivityRelevant({ ...crossProject, event_type: eventType }, "/projects/AUTO", projects)).toBe(true)
    }

    expect(isActivityRelevant({ ...crossProject, event_type: "ticket.created", payload: { status: "backlog" } }, "/projects/AUTO", projects, undefined, rootTriageTicketIds)).toBe(false)
    expect(isActivityRelevant({ ...crossProject, event_type: "ticket.created", payload: { status: "triage" } }, "/projects/AUTO", projects, undefined, rootTriageTicketIds)).toBe(true)
    expect(isActivityRelevant({ ...crossProject, event_type: "ticket.transitioned", payload: { status: { from: "backlog", to: "ready" } } }, "/projects/AUTO", projects, undefined, rootTriageTicketIds)).toBe(false)
    expect(isActivityRelevant({ ...crossProject, event_type: "ticket.transitioned", payload: { status: { from: "triage", to: "ready" } } }, "/projects/AUTO", projects, undefined, rootTriageTicketIds)).toBe(true)
    expect(isActivityRelevant({ ...crossProject, event_type: "ticket.transitioned", payload: { status: { from: "ready", to: "triage" } } }, "/projects/AUTO", projects, undefined, rootTriageTicketIds)).toBe(true)
    expect(isActivityRelevant({ ...crossProject, event_type: "ticket.updated" }, "/projects/AUTO", projects, undefined, rootTriageTicketIds)).toBe(false)
    expect(isActivityRelevant({ ...event, event_type: "ticket.updated" }, "/projects/AUTO", projects, undefined, rootTriageTicketIds)).toBe(true)
  ...
  it("preserves current project, ticket, drawer, and triage route relevance", () => {
    ...
    expect(isActivityRelevant(event, "/projects/AUTO", projects, undefined, triageIds)).toBe(true)
    ...
    expect(isActivityRelevant({ ...event, event_type: "project.updated", ticket_id: null }, "/projects", projects)).toBe(true)
    expect(isActivityRelevant({ ...event, ticket_id: "33333333-3333-4333-8333-333333333333" }, "/projects/OTHER", projects)).toBe(false)
    expect(isActivityRelevant(event, "/triage", projects, undefined, triageIds)).toBe(true)
    expect(isActivityRelevant({ ...event, ticket_id: "33333333-3333-4333-8333-333333333333" }, "/triage", projects, undefined, triageIds)).toBe(false)
  })
  ...
  it("invalidates a direct detail page for its visible relationships and newly created relationships only", () => {
    ...
    expect(isActivityRelevant({ ...event, event_type: "ticket.created", ticket_id: unrelated, payload: { parent_ticket_id: unrelated, status: "backlog" } }, "/tickets/AUTO-1", projects, current)).toBe(false)
