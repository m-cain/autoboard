...
export const activityStream = (
  ...
  Stream.async((emit) => {
    ...
    const schedule =
      options.schedule ??
      ((reconnect, delay) => window.setTimeout(reconnect, delay));
    ...
    const retire = (
      ...
      onActivity: (event: MessageEvent<string>) => void,
    ) => {
        ...
        try {
          ...
          if (
            (reportedId !== undefined &&
              (!Number.isSafeInteger(reportedId) ||
                reportedId !== decoded.id)) ||
            decoded.id <= greatestId
          )
            return;
