...
class FakeEventSource implements EventSourceLike {
  ...
  fail() { this.onerror?.() }
  listener(type: string) { return [...(this.listeners.get(type) ?? [])][0] }
}
...
describe("activityStream", () => {
  ...
  it("detaches retired sources so queued callbacks cannot advance the replay cursor", async () => {
    ...
    const reconnects: Array<{ readonly work: () => void; readonly delay: number }> = []
    const seen: number[] = []
    const fiber = Effect.runFork(Stream.runForEach(activityStream({
      createEventSource: (url) => { urls.push(url); const source = new FakeEventSource(); sources.push(source); return source },
      schedule: (work, delay) => { reconnects.push({ work, delay }); return reconnects.length },
      cancel: () => undefined,
    }), (activity) => Effect.sync(() => seen.push(activity.id))))
    fibers.push(fiber)
    await settle()
    ...
    retiredActivity({ data: JSON.stringify({ ...event, id: 99 }), lastEventId: "99" } as MessageEvent<string>)
    retiredError()
    ...
    activeActivity({ data: JSON.stringify({ ...event, id: 42 }), lastEventId: "42" } as MessageEvent<string>)
    activeError()
