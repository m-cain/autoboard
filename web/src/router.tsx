...
type RootData = {
  readonly projects: {
    readonly active: readonly Project[];
    readonly archived: readonly Project[];
  };
  readonly triage: TicketListData;
};
export type TicketRouteData = {
...
const ErrorPage = () => {
  ...
  return (
    <main className="route-error">
      <p className="eyebrow">{missing ? "Not found" : "Unavailable"}</p>
      <h1>
        {missing ? "This view does not exist" : "Autoboard is unavailable"}
      </h1>
      <p>
        {unavailable
          ? "The local server is starting or unavailable. Try again shortly."
          : "The current board could not be loaded."}
      </p>
...
const ProjectsRoute = () => (
  <ProjectsPage projects={(useRouteLoaderData("root") as RootData).projects} />
);
const TriageRoute = () => {
  const root = useRouteLoaderData("root") as RootData;
  return (
    <TriagePage tickets={root.triage.tickets} projects={root.projects.active} />
  );
};
const BoardRoute = () => {
  ...
  return (
    <ProjectBoardPage
      board={useLoaderData() as ProjectBoard}
      restoreSnapshot={snapshot}
      onSnapshotRestored={
        snapshot ? () => clearBoardSnapshot(location.key) : undefined
      }
    />
  );
};
const ticketIdentifier = (
  ...
  if (!params.identifier)
    throw new HttpError({ status: 404, message: "Ticket not found" });
  return params.identifier;
};
type DrawerLocationState = {
  readonly backgroundLocation: unknown;
  readonly drawerDepth?: unknown;
  readonly originIdentifier?: unknown;
  readonly boardSnapshotKey?: unknown;
};
...
const TicketRoute = () => {
  ...
  const originIdentifier =
    typeof drawerState.originIdentifier === "string"
      ? drawerState.originIdentifier
      : undefined;
  const drawerDepth =
    typeof drawerState.drawerDepth === "number" &&
    Number.isInteger(drawerState.drawerDepth) &&
    drawerState.drawerDepth > 0
      ? drawerState.drawerDepth
      : 1;
  const snapshot =
    typeof drawerState.boardSnapshotKey === "string"
      ? boardSnapshot(drawerState.boardSnapshotKey)
      : undefined;
  const snapshotKey =
    typeof drawerState.boardSnapshotKey === "string"
      ? drawerState.boardSnapshotKey
      : undefined;
  return (
    <div className="ticket-drawer-layer">
      <div aria-hidden="true">
        <ProjectBoardPage board={data.board} restoreSnapshot={snapshot} />
      </div>
      <TicketDrawer
        closeTo={closeTo}
        originIdentifier={originIdentifier}
        drawerDepth={drawerDepth}
        focusKey={data.ticket.id}
        boardSnapshotKey={snapshotKey}
      >
...
const CanceledRoute = () => {
  const { project, tickets } = useLoaderData() as {
...
const NotFoundPage = () => (
  <section className="route-error">
    ...
    <h1>This view does not exist</h1>
...
export const createAppRoutes = (client: ApiClientService): RouteObject[] => {
  ...
  const fromApi = <A, E>(
    get: (service: ApiClientService) => Effect.Effect<A, E>,
  ) => run(Effect.flatMap(ApiClient, get));
  const rootLoader = async ({
    ...
    const [projects, triage] = await Promise.all([
      fromApi((service) => service.listProjects(request.signal)),
      fromApi((service) => service.listTriage(request.signal)),
    ]);
      ...
      children: [
        ...
        { path: "projects", Component: ProjectsRoute },
        {
          ...
          loader: ({ params, request }) =>
            fromApi((service) =>
              service.getProjectBoard(projectKey(params), request.signal),
            ),
          Component: BoardRoute,
        },
        {
          path: "projects/:key/canceled",
          loader: async ({ params, request }) => {
            const key = projectKey(params);
            const [board, canceled] = await Promise.all([
              fromApi<ProjectBoard, unknown>((service) =>
                service.getProjectBoard(key, request.signal),
              ),
              fromApi<TicketListData, unknown>((service) =>
                service.getCanceledTickets(key, request.signal),
              ),
            ]);
        ...
        },
        {
          ...
          loader: async ({ params, request }): Promise<TicketRouteData> => {
            const ticket = await fromApi<TicketDetail, unknown>((service) =>
              service.getTicket(ticketIdentifier(params), request.signal),
            );
            const board = await fromApi<ProjectBoard, unknown>((service) =>
              service.getProjectBoard(ticket.project.key, request.signal),
            );
            return { ticket, board };
          },
...
export const router = createBrowserRouter(createAppRoutes(createApiClient()), {
