...
export const TicketCard = ({ ticket }: { readonly ticket: TicketSummary }) => {
  ...
  return <article className="ticket-card">
    <Link className="ticket-card__title" data-ticket-identifier={ticket.identifier} to={`/tickets/${encodeURIComponent(ticket.identifier)}`} state={boardOrigin ? { backgroundLocation: location, drawerDepth: 1, originIdentifier: ticket.identifier, boardSnapshotKey: location.key } : undefined} onClickCapture={(event) => {
      if (!boardOrigin || event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return
      const scroller = event.currentTarget.closest(".board-page")?.querySelector<HTMLElement>("[data-kanban-scroll]")
      saveBoardSnapshot(location.key, { scrollX: window.scrollX, scrollY: window.scrollY, kanbanScrollLeft: scroller?.scrollLeft ?? 0 })
    }}>
      {ticket.title}
    </Link>
