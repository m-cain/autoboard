...
describe("ApiClient", () => {
  it("uses GET and decodes the exact projects response", async () => {
    const fetch = vi
      .fn()
      .mockResolvedValue(
        new Response(JSON.stringify(projectResponse), { status: 200 }),
      );
    const client = createApiClient({ fetch, sleep: async () => undefined });
    await expect(Effect.runPromise(client.listProjects())).resolves.toEqual(
      projectResponse,
    );
  ...
  it("does not retry non-503 HTTP or malformed responses", async () => {
    ...
    const client = createApiClient({ fetch, sleep: async () => undefined });
    await expect(Effect.runPromise(client.listProjects())).rejects.toThrow(
      "HTTP 404",
    );
  ...
  it("uses only GET requests for every browser read endpoint", async () => {
    ...
    const board = {
      project: projectResponse.active[0],
      columns: { backlog: [], ready: [ticket], in_progress: [], done: [] },
    };
    const fetch = vi.fn((path: RequestInfo | URL, _init?: RequestInit) => {
      const requestPath = String(path);
      const payload = requestPath.includes("board")
        ? board
        : requestPath.includes("canceled") || requestPath.endsWith("triage")
          ? { tickets: [ticket] }
          : requestPath.includes("tickets/AUTO-1")
            ? {
                ...ticket,
                project: projectResponse.active[0],
                parent: null,
    ...
    await Promise.all([
      ...
      Effect.runPromise(client.listTriage()),
      Effect.runPromise(client.getProjectBoard("AUTO")),
      Effect.runPromise(client.getCanceledTickets("AUTO")),
      Effect.runPromise(client.getTicket("AUTO-1")),
    ]);

    expect(fetch.mock.calls.map((args) => args[1]?.method)).toEqual([
      "GET",
      "GET",
      "GET",
      "GET",
  ...
  it("propagates a request signal and stops promptly when fetch is aborted", async () => {
    ...
    const request = run(
      Effect.flatMap(ApiClient, (api) => api.listProjects(controller.signal)),
    );
    controller.abort();
