...
const drawerState = (state: unknown): DrawerState | undefined => {
  ...
  if (
    ...
    !/^\/projects\/[^/]+$/.test(background.pathname)
  )
    return undefined;
...
const TicketLinks = ({
  ...
  tickets.length === 0 ? (
    ...
    <ul className="detail-ticket-links">
      {tickets.map((ticket) => (
        <li key={ticket.id}>
          <Link
            to={ticketPath(ticket.identifier)}
            state={
              state
                ? { ...state, drawerDepth: (state.drawerDepth ?? 1) + 1 }
                : undefined
            }
          >
...
export const TicketDetailPage = ({
  ...
  return (
    <article className="page ticket-detail">
      <header className="page-heading">
        ...
        {ticket.labels.length > 0 ? (
          <ul className="label-list" aria-label="Labels">
            {ticket.labels.map(
              (label: { readonly id: string; readonly name: string }) => (
      ...
      {ticket.parent ? (
        <section className="detail-section" aria-labelledby="parent-heading">
      ...
      <section className="detail-section" aria-labelledby="blockers-heading">
        <h2 id="blockers-heading">Blockers</h2>
        <TicketLinks
          tickets={ticket.blockers}
          empty="No unresolved blockers"
          state={state}
        />
      </section>
      <section className="detail-section" aria-labelledby="blocked-heading">
        <h2 id="blocked-heading">Blocks</h2>
        <TicketLinks
          tickets={ticket.blocked_tickets}
          empty="Does not block other tickets"
          state={state}
        />
      </section>
