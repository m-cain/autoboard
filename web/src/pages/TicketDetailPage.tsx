...
const TicketLinks = ({ tickets, empty }: { readonly tickets: readonly TicketSummary[]; readonly empty: string }) => (
  tickets.length === 0 ? <p className="empty-state">{empty}</p> : <ul className="detail-ticket-links">
    {tickets.map((ticket) => <li key={ticket.id}><Link to={ticketPath(ticket.identifier)}>{ticket.identifier} · {ticket.title}</Link></li>)}
  </ul>
)

export const TicketDetailPage = ({ ticket }: { readonly ticket: TicketDetail }) => (
  <article className="page ticket-detail">
    ...
    {ticket.parent ? <section className="detail-section" aria-labelledby="parent-heading"><h2 id="parent-heading">Parent</h2><TicketLinks tickets={[ticket.parent]} empty="No parent" /></section> : null}
    <section className="detail-section" aria-labelledby="blockers-heading"><h2 id="blockers-heading">Blockers</h2><TicketLinks tickets={ticket.blockers} empty="No unresolved blockers" /></section>
    <section className="detail-section" aria-labelledby="blocked-heading"><h2 id="blocked-heading">Blocks</h2><TicketLinks tickets={ticket.blocked_tickets} empty="Does not block other tickets" /></section>
    <section className="detail-section" aria-labelledby="subtasks-heading"><h2 id="subtasks-heading">Subtasks</h2><TicketLinks tickets={ticket.subtasks} empty="No subtasks" /></section>
