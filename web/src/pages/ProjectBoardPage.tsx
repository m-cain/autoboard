...
export const ProjectBoardPage = ({
  ...
  readonly restoreSnapshot?: BoardSnapshot;
  ...
  useLayoutEffect(() => {
    ...
    if (scroller.current)
      scroller.current.scrollLeft = restoreSnapshot.kanbanScrollLeft;
  ...
  return (
    <section className="page board-page">
      ...
      <div ref={scroller} className="kanban-scroll" data-kanban-scroll>
        <div className="kanban-board">
          {columns.map(({ key, title }) => (
            <BoardColumn key={key} title={title} tickets={board.columns[key]} />
          ))}
        </div>
      </div>
    </section>
