...
export class RpcClient {
  ...
  async call<S extends Schema.Schema.Any>(
    ...
    if (!this.socket)
      throw this.failed ?? new RpcConnectionError("RPC client is disconnected");
      ...
      try {
        ...
        reject(
          error instanceof Error
            ? error
            : new RpcConnectionError("Unable to send RPC request"),
        );
      }
    });
  }
      ...
      const onError = (error: Error) => {
        ...
        finish(
          new RpcConnectionError(
            `Unable to connect to RPC socket: ${error.message}`,
          ),
        );
      };
      const onClose = () => {
  ...
  private initialize(): Promise<Schema.Schema.Type<typeof SessionInitialize>> {
    const id = this.allocateId();
    return new Promise<Schema.Schema.Type<typeof SessionInitialize>>(
      (resolve, reject) => {
        ...
        try {
          ...
          reject(
            error instanceof Error
              ? error
              : new RpcConnectionError("Unable to initialize RPC session"),
          );
        }
      },
    );
  ...
  private write(message: unknown): void {
    ...
    if (payload.length > MAX_FRAME_BYTES) {
      throw new RpcProtocolError(
        `RPC request exceeds ${MAX_FRAME_BYTES} bytes`,
      );
    }
  ...
  private receive(socket: Socket, chunk: Buffer): void {
    ...
    while (this.receiveBuffer.length >= 4) {
      ...
      if (size > MAX_FRAME_BYTES) {
        this.failProtocol(
          socket,
          `RPC response exceeds ${MAX_FRAME_BYTES} bytes`,
        );
        return;
      }
      ...
      try {
        ...
        this.failProtocol(
          socket,
          error instanceof Error ? error.message : "Malformed RPC response",
        );
        return;
      }
    }
  ...
  private response(message: Record<string, unknown>): void {
    ...
    if (initializing) {
      ...
      if ("error" in message) {
        try {
          ...
          initializing.reject(
            new RpcError(error.code, error.message, error.data),
          );
        } catch (error) {
          initializing.reject(
            error instanceof Error
              ? error
              : new RpcProtocolError("Invalid JSON-RPC error response"),
          );
        }
        ...
        try {
          initializing.resolve(
            Schema.decodeUnknownSync(
              SessionInitialize as unknown as Schema.Schema<
                Schema.Schema.Type<typeof SessionInitialize>,
                unknown,
                never
              >,
            )(message.result),
          ...
          initializing.reject(
            new RpcProtocolError(
              `Invalid session.initialize response: ${String(error)}`,
            ),
          );
        }
      } else
        initializing.reject(
          new RpcProtocolError("Invalid session.initialize response"),
        );
      return;
    }
    ...
    try {
      pending.resolve(
        Schema.decodeUnknownSync(
          pending.schema as Schema.Schema<unknown, unknown, never>,
        )(message.result),
      );
    } catch (error) {
      pending.reject(
        new RpcProtocolError(
          `RPC result failed contract decoding: ${String(error)}`,
        ),
      );
    }
  }

  private decodeErrorEnvelope(
    ...
    try {
      ...
      if (envelope.id !== expectedId)
        throw new RpcProtocolError(
          "JSON-RPC error response id does not match request",
        );
      return envelope.error;
    } catch (error) {
      if (error instanceof RpcProtocolError) throw error;
      throw new RpcProtocolError(
        `Invalid JSON-RPC error envelope: ${String(error)}`,
      );
    }
  }
  ...
  private disconnected(socket: Socket): void {
    if (socket !== this.socket || this.closed) return;
    this.socket = undefined;
    const initializationError = new RpcConnectionError(
      "RPC connection closed during initialization",
    );
    ...
    if (this.disconnects >= 2 || this.failed) {
      const error =
        this.failed ??
        new RpcConnectionError("RPC connection failed after its retry");
      this.failed = error;
      this.rejectAll(error);
      return;
    }
  ...
  private async reconnect(): Promise<void> {
    try {
      ...
      const failure =
        error instanceof Error
          ? error
          : new RpcConnectionError("RPC reconnection failed");
      if (!this.closed) {
        this.failed = failure;
