...
const project = { id, key: "AUTO", name: "Autoboard", description: "", state: "active", revision: 2, inserted_at: timestamp, updated_at: timestamp }
const summary = {
  id, identifier: "AUTO-1", project_id: id, title: "Build it", description: "", status: "ready", priority: "medium", assignee: "codex", revision: 2,
  parent_ticket_id: null, labels: [{ id: otherId, name: "feature", project_id: id }], blocked: false, comment_count: 1, attachment_count: 1, inserted_at: timestamp, updated_at: timestamp,
}
const attachment = { id: attachmentId, ticket_id: id, project_id: id, original_filename: "note.txt", media_type: "text/plain", byte_size: 5, sha256: "a".repeat(64), actor: "codex", inserted_at: timestamp }
const ticketDetail = {
  ...summary, project, parent: null, subtasks: [], blockers: [], blocked_tickets: [],
  comments: [{ id, ticket_id: id, project_id: id, body: "note", actor: "codex", inserted_at: timestamp }],
  attachments: [attachment], activity: [{ id: 1, event_type: "ticket.created", actor: "codex", project_id: id, ticket_id: id, payload: {}, inserted_at: timestamp }],
}
const attachmentInline = { ...attachment, content: "hello" }
const attachmentWrite = { ...attachment, ticket_revision: 3 }
const comment = { id, ticket_id: id, project_id: id, body: "note", actor: "codex", inserted_at: timestamp, ticket_revision: 3 }

const inputs = new Map<string, Record<string, unknown>>([
  ["list_projects", {}], ["get_project_board", { project_id: "AUTO" }], ["search_tickets", { query: "build", limit: 10 }],
  ["get_ticket", { ticket_id: "AUTO-1" }], ["list_actionable_tickets", {}], ["read_attachment", { attachment_id: attachmentId }],
...
const responseFor = (method: string): Record<string, unknown> => {
  switch (method) {
    case "projects.list": return { active: [project], archived: [] }
    case "tickets.board": return { project, columns: { backlog: [], ready: [summary], in_progress: [], done: [] } }
    case "tickets.search": case "tickets.actionable": return { tickets: [summary] }
    case "tickets.get": return ticketDetail
    case "attachments.read": return attachmentInline
    case "comments.add": return comment
    case "attachments.add_from_path": return attachmentWrite
    case "projects.create": case "projects.update": case "projects.archive": case "projects.restore": return project
...
describe("Autoboard MCP tool registry", () => {
  test("publishes exact descriptions, strict JSON schemas, bounded inputs, and annotations for all 17 tools", async () => {
    ...
    for (const [index, tool] of toolRegistry.entries()) {
      ...
      if (tool.name === "list_projects") expect(Object.keys(wire.inputSchema.properties ?? {})).toHaveLength(0)
      else expect(Object.keys(wire.inputSchema.properties ?? {}).length).toBeGreaterThan(0)
      expect(tool.inputSchema.safeParse(inputs.get(tool.name)).success).toBe(true)
      expect(tool.outputSchema.safeParse(responseFor(tool.rpcMethod)).success).toBe(true)
      for (const forbidden of ["actor", "scope", "method", "sql", "delete"]) {
        expect(tool.inputSchema.safeParse({ ...inputs.get(tool.name), [forbidden]: "forbidden" }).success).toBe(false)
      }
      expect(wire.annotations?.readOnlyHint).toBe(index < 6)
      expect(wire.annotations?.openWorldHint).toBe(false)
      expect(wire.annotations?.destructiveHint).toBe(["archive_project", "remove_dependency"].includes(tool.name))
    }
    ...
    expect(toolRegistry.find((tool) => tool.name === "search_tickets")!.inputSchema.parse({}).limit).toBe(25)
    for (const name of ["update_project", "archive_project", "restore_project", "update_ticket", "transition_ticket", "add_dependency", "remove_dependency"]) {
      ...
      const { expected_revision: _expectedRevision, ...withoutRevision } = input
      expect(toolRegistry.find((tool) => tool.name === name)!.inputSchema.safeParse(withoutRevision).success).toBe(false)
    }
    await client.close()
  })
  ...
  test("runs every tool through the official MCP SDK with contract-valid structured output", async () => {
    ...
    for (const name of names) {
      const result = await client.callTool({ name, arguments: inputs.get(name) })
      expect(result.isError).not.toBe(true)
      expect(result.content).toEqual(expect.arrayContaining([expect.objectContaining({ type: "text" })]))
      expect(result.structuredContent).toEqual(responseFor(rpcMethods[names.indexOf(name)]!))
    }
    expect(rpc.calls.map((call) => call.method)).toEqual(rpcMethods)
    expect(rpc.calls.map((call) => call.mode)).toEqual(names.map((_, index) => index < 6 ? "read" : "write"))
    await client.close()
  })
  ...
  test("returns repairable domain, protocol, and indeterminate write errors over MCP", async () => {
    const rpc = new FakeClient()
    const { client } = await connected(rpc)
    rpc.failure = new RpcError(-32010, "stale", { kind: "revision_conflict", message: "stale", fields: { expected_revision: ["is stale"] }, current: project })
    await expect(client.callTool({ name: "update_ticket", arguments: inputs.get("update_ticket") })).resolves.toMatchObject({
      isError: true, content: [{ text: expect.stringContaining("expected_revision") }],
    })
    rpc.failure = new RpcProtocolError("bad response")
    await expect(client.callTool({ name: "get_ticket", arguments: inputs.get("get_ticket") })).resolves.toMatchObject({
      isError: true, content: [{ text: expect.stringContaining("protocol") }],
    })
    rpc.failure = new IndeterminateWriteError("tickets.update")
    await expect(client.callTool({ name: "update_ticket", arguments: inputs.get("update_ticket") })).resolves.toMatchObject({
      isError: true, content: [{ text: expect.stringContaining("Do not retry") }],
    })
    await client.close()
  })
})
